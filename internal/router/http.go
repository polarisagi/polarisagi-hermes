...
func ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	incomingProtocol := getIncomingProtocol(r.URL.Path)
	if incomingProtocol == "unknown" {
		http.Error(w, `{"error": "Unsupported protocol endpoint"}`, http.StatusBadRequest)
		return
	}

	modelName := extractModelName(bodyBytes, incomingProtocol)
	if modelName == "" {
		http.Error(w, `{"error": "Missing model field in request"}`, http.StatusBadRequest)
		return
	}

	// Wait for an available node via router
	ctx, cancel := context.WithTimeout(r.Context(), 180*time.Second)
	defer cancel()

	atomic.AddInt32(&webapi.WaitingCount, 1)
	dest, err := MatchAndAcquireRoute(ctx, modelName)
	atomic.AddInt32(&webapi.WaitingCount, -1)

	if err != nil || dest == nil {
		slog.Error("路由分发失败或队列超时", "trace_id", traceID, "model", modelName, "error", err)
		http.Error(w, fmt.Sprintf("Polaris Gateway: No active routes available for model %s or queue timeout", modelName), http.StatusServiceUnavailable)
		return
	}
	...
	targetProvider := dest.Node.Provider
	translatorKey := fmt.Sprintf("%s_to_%s", incomingProtocol, targetProvider)
	...
	slog.Info("🔗 [路由转发]", "trace_id", traceID, "incoming", incomingProtocol, "target_provider", targetProvider, "target_node", dest.Node.Name, "original_model", modelName, "target_model", dest.TargetModel)

	// Inject usage if needed for OpenAI streams
