...
- **极简模式 (Intelligent / Semantic Mapping)**：
  主打"开箱即用"。网关提取源模型名（如 `gpt-4o`），通过内部语义特征库自动泛化（识别为"旗舰型模型"）。当路由命中异构目标节点时，Translator 自动将其映射为目标协议的等价旗舰模型（如 `claude-3-5-sonnet-latest` 或 `gemini-2.5-pro`），对客户端完全透明。
  
- **专业模式 (Pro / Deterministic 1-to-1 Mapping)**：
  主打"绝对控制"。用户可在路由规则中手动配置强制映射（Hard-coded Mapping），支持精确匹配或正则模式拦截。适用于将内部代号、旧模型 ID 强制引流至特定微调模型或本地部署模型的场景。**专业模式优先级高于极简模式。**
