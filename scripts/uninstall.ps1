...
Write-Host "🗑️ 正在卸载 Polaris Gateway (Windows)..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "⚠️ 注意: 您的数据库和配置数据仍保留在 $env:USERPROFILE\.polaris-gateway 目录中。" -ForegroundColor Yellow
Write-Host "如果您想彻底清理所有数据（这会删除所有配置和账单记录），请手动执行：" -ForegroundColor Yellow
Write-Host "Remove-Item -Path $env:USERPROFILE\.polaris-gateway -Recurse -Force" -ForegroundColor Yellow
Write-Host ""
